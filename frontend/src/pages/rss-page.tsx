...
const itemStatuses = [{ value: "", label: "全部条目" }, { value: "pending", label: "待处理" }, { value: "queued", label: "已入队" }, { value: "skipped", label: "已跳过" }, { value: "baseline", label: "历史基线" }, { value: "needs_attention", label: "需处理" }];
const jobStatuses = [{ value: "", label: "全部投递状态" }, { value: "queued", label: "已加入队列" }, { value: "held", label: "已挂起" }, { value: "waiting", label: "等待条件" }, { value: "retry_wait", label: "等待重试" }, { value: "reconciling", label: "正在确认投递" }, { value: "submitted", label: "已提交" }, { value: "already_present", label: "已存在" }, { value: "needs_attention", label: "需处理" }, { value: "failed", label: "投递失败" }, { value: "cancelled", label: "已取消" }];
...
export function RssPage() {
  ...
  function toggleFeed(feed: RssFeed) {
    ...
    const execute = () => mutation(`feed-${feed.id}-${action}-${feed.version}`, (id) => rssApi.post(`/feeds/${feed.id}/${action}`, { expected_version: feed.version, request_id: id }), feed.enabled ? "订阅源已暂停，尚未提交的任务已挂起。" : "订阅源已恢复。首次检查建立新的历史基线，然后收取后续新内容。");
    if (feed.enabled) void execute(); else setConfirmation({ title: "恢复订阅源", description: "继续已有队列；恢复后的首次检查会将暂停期间出现的条目记为历史基线，再收取后续新发现的内容。需要的历史资源可单独选择补下。", label: "恢复检查", execute });
  }
  function toggleRule(rule: RssRule) {
    const action = rule.enabled ? "pause" : "resume";
    const execute = () => mutation(`rule-${rule.id}-${action}-${rule.version}`, (id) => rssApi.post(`/rules/${rule.id}/${action}`, { expected_version: rule.version, request_id: id }), rule.enabled ? "规则已暂停，尚未提交的任务已挂起。" : "规则已启用，仅处理此后新发现的内容。");
    if (rule.enabled) void execute(); else setConfirmation({ title: "启用下载规则", description: "继续已有队列，仅自动处理此后新发现的内容；暂停期间的资源需要在订阅源中选择补下。", label: "启用规则", execute });
  }
  function archive(kind: "feeds" | "rules", record: RssFeed | RssRule) {
    setConfirmation({ title: `归档${kind === "feeds" ? "订阅源" : "下载规则"}`, description: `归档“${record.name}”将停止新工作，并取消确认未提交的任务。已经发出的提交会继续对账；历史记录、下载器中的种子和文件会保留。`, label: "确认归档", dangerous: true, execute: () => mutation(`${kind}-${record.id}-archive-${record.version}`, async (id) => { await rssApi.archive(`/${kind}/${record.id}`, { expected_version: record.version, request_id: id }); if (kind === "feeds" && route.feed === record.id) { setSelectedFeed(null); go({ feed: null, page: 1 }); } }, "已归档，成功投递历史仍可查询。") });
  }
  async function check(feed: RssFeed) {
    await mutation(`feed-${feed.id}-check-${feed.version}`, async (id) => { const run = await rssApi.post<RssRun>(`/feeds/${feed.id}/check`, { expected_version: feed.version, request_id: id }); setWatchedRun(run); }, "检查请求已保存，系统会抓取来源并按启用规则处理新条目。");
  }
  function jobAction(action: "retry" | "reconcile" | "cancel", job: RssJob) {
    const execute = () => mutation(`job-${job.id}-${action}-${job.version}`, (id) => rssApi.post(`/downloads/${job.id}/${action}`, { expected_version: job.version, request_id: id }), action === "retry" ? "重试请求已保存。" : action === "reconcile" ? "对账请求已保存，正在确认下载器中的实际状态。" : "未提交任务已取消。");
    if (action === "cancel") setConfirmation({ title: "取消未提交任务", description: `取消“${job.title}”后将停止本次投递。系统会先确认任务仍未提交，不会删除下载器中的内容。`, label: "确认取消任务", dangerous: true, execute }); else void execute();
  }
  ...
  return <div className="space-y-5" data-rss-page="true">
    ...
    {watchedRun && !editing && <div className="flex flex-wrap items-center gap-3 rounded-xl border border-border bg-card p-4 text-xs"><Status status={watchedRun.status} /><span className="flex-1 leading-6">检查 #{watchedRun.id} · 发现 {watchedRun.new_count} 条新资源 · 入队 {watchedRun.queued_count} 条 · 待处理 {watchedRun.pending_count} 条{watchedRun.message ? ` · ${watchedRun.message}` : ""}</span><Button className="h-11 px-3" variant="outline" onClick={() => setRunsOpen(true)}>查看处理记录</Button></div>}
    {editing ? editLoading ? <ListSkeleton /> : route.edit !== "new" && !editRule ? <Empty title="无法载入此规则" action={<Button className="h-11" variant="outline" onClick={() => selectTab("rules")}>返回下载规则</Button>}>规则可能已归档，或暂时无法连接服务。</Empty> : <RuleEditor key={route.edit} rule={editRule} feeds={feeds} downloaders={downloaders} feedId={route.feed ?? undefined} onBack={() => selectTab("rules")} onSaved={() => { setNotice("规则已保存。已有条目不会因为保存规则而自动补下。"); void refreshOptions(); selectTab("rules"); }} /> : <div role="tabpanel" id="rss-tab-panel" aria-labelledby={`rss-tab-${route.tab}`} className="space-y-4">
      {route.tab === "feeds" && route.feed && <div className="space-y-3"><Button className="-ml-3 h-11 border-transparent bg-transparent px-3" variant="outline" onClick={() => { setSelectedFeed(null); go({ feed: null, q: "", status: "", page: 1 }); }}><ArrowLeft />返回订阅源</Button>{feed && <><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words text-xl font-semibold">{feed.name}</h2><p className="mt-2 text-xs leading-6 text-muted">{feed.site_name || "独立订阅源"} · {feed.enabled ? "已启用" : "已暂停"} · 每 {feed.interval_minutes} 分钟检查 · {feed.item_count} 条已收集</p></div><div className="flex flex-wrap gap-2"><Button className="h-11 px-3" variant="outline" disabled={!!busy || !feed.enabled} onClick={() => void check(feed)}><RefreshCw />立即检查</Button><Button className="h-11 px-3" onClick={() => openRule("new", feed.id)}><Plus />为此源创建规则</Button></div></div><p className="break-all text-xs text-muted">{feed.url_display}</p>{!feed.initialized_at && <p className="rounded-lg bg-accent p-3 text-sm leading-6">等待首次成功检查以建立历史基线。首次收集的条目不会自动下载。</p>}{!feed.enabled && <p className="text-sm text-muted">源已暂停，未提交任务已挂起。<button className="min-h-11 px-2 font-medium text-primary underline" onClick={() => toggleFeed(feed)}>恢复订阅源</button></p>}{feed.last_error && <ErrorBox action={<a className="inline-flex min-h-11 items-center text-primary underline" href="#/sites">查看站点配置<ArrowUpRight className="ml-1 size-4" /></a>}>{feed.last_error}</ErrorBox>}{!rules.some((rule) => rule.feed_ids.includes(feed.id) && rule.enabled) && <p className="text-xs leading-6 text-muted">已收集条目，尚未开启自动下载。创建并启用规则后，只自动收取后续新发现的内容。</p>}</>}</div>}
      <div className="grid gap-3 sm:grid-cols-2 xl:flex xl:items-end"><div className="min-w-0 xl:flex-1"><label className="mb-2 block text-xs font-medium" htmlFor="rss-search">{route.tab === "feeds" && !route.feed ? "搜索订阅源" : route.tab === "rules" ? "搜索规则" : "搜索资源"}</label><div className="relative"><Search className="pointer-events-none absolute left-3 top-3.5 size-4 text-muted" /><Input id="rss-search" className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={route.tab === "feeds" && !route.feed ? "按名称查找" : route.tab === "rules" ? "按规则名称查找" : "按资源标题查找"} /></div></div>{route.tab === "feeds" && !route.feed ? <div className="min-w-0 xl:w-44"><label className="mb-2 block text-xs font-medium" htmlFor="rss-site-filter">关联站点</label><Select id="rss-site-filter" value={route.site} onChange={(value) => go({ site: value, page: 1 })} options={[{ value: "", label: "全部站点" }, ...sites.map((site) => ({ value: String(site.id), label: site.name }))]} /></div> : route.tab !== "feeds" ? <div className="min-w-0 xl:w-44"><label className="mb-2 block text-xs font-medium" htmlFor="rss-source-filter">订阅源</label><Select id="rss-source-filter" value={route.source} onChange={(value) => go({ source: value, page: 1 })} options={[{ value: "", label: "全部来源" }, ...feeds.map((record) => ({ value: String(record.id), label: record.name }))]} /></div> : null}{route.tab === "downloads" && <div className="min-w-0 xl:w-44"><label className="mb-2 block text-xs font-medium" htmlFor="rss-target-filter">下载器</label><Select id="rss-target-filter" value={route.target} onChange={(value) => go({ target: value, page: 1 })} options={[{ value: "", label: "全部下载器" }, ...downloaders.map((record) => ({ value: String(record.id), label: record.name }))]} /></div>}<div className="min-w-0 xl:w-48"><label className="mb-2 block text-xs font-medium" htmlFor="rss-status-filter">{route.tab === "downloads" ? "投递状态" : "状态"}</label><Select id="rss-status-filter" value={route.status} onChange={(value) => go({ status: value, page: 1 })} options={route.tab === "downloads" ? jobStatuses : route.feed && route.tab === "feeds" ? itemStatuses : feedStatuses} /></div>{hasFilters && <Button className="h-11 px-3" variant="outline" onClick={clearFilters}><ListFilter />清除筛选</Button>}</div>
    ...
    {runsOpen && <Dialog open title="检查与处理记录" description="抓取、匹配与入队分别计数。检查完成后，待确认属性或超出配额的资源仍可能继续处理。" onClose={() => setRunsOpen(false)} panelClassName="max-w-3xl"><div className="p-4 sm:p-6">{runs.items.length === 0 ? <Empty title="暂无检查记录">添加并启用订阅源后，检查结果会保存在这里。</Empty> : <div className="divide-y divide-border">{runs.items.map((run) => <article key={run.id} className="space-y-2 py-4"><div className="flex flex-wrap items-center justify-between gap-2"><h4 className="text-sm font-semibold">{run.kind === "backfill" ? "历史补下" : "订阅源检查"} #{run.id}{run.feed_id ? ` · ${feeds.find((record) => record.id === run.feed_id)?.name ?? "已归档来源"}` : ""}</h4><Status status={run.status} /></div><p className="text-xs leading-6 text-muted">读取 {run.item_count} 条 · 新发现 {run.new_count} 条 · 入队 {run.queued_count} 条 · 待处理 {run.pending_count} 条</p><p className="text-xs text-muted">开始 {rssDate(run.started_at)} · {run.finished_at ? `结束 ${rssDate(run.finished_at)}` : "仍在处理"}</p>{run.message && <p className="break-words text-sm leading-6">{run.message}</p>}</article>)}</div>}<Pager page={runPage} total={runs.total} size={runs.page_size} onChange={setRunPage} /></div></Dialog>}
