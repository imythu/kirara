...
import { SubscriptionEditor } from "@/components/rss/subscription-editor";
...
type Route = { tab: Tab; feed: number | null; job: number | null; edit: string; subscription: string; q: string; status: string; site: string; source: string; target: string; page: number };
...
function readRoute(): Route {
  ...
  return { tab: tab === "rules" || tab === "downloads" ? tab : "feeds", feed: number("feed"), job: number("job"), edit: search.get("edit") ?? "", subscription: search.get("subscription") ?? "", q: search.get("q") ?? "", status: search.get("status") ?? "", site: search.get("site") ?? "", source: search.get("source") ?? "", target: search.get("target") ?? "", page: number("page") ?? 1 };
}
...
export function RssPage() {
  ...
  const [subscription, setSubscription] = useState<{ feed: RssFeed; rule: RssRule | null } | null>(null);
  ...
  useEffect(() => {
    setSubscription(null); setSubscriptionError("");
    if (!route.subscription || route.subscription === "new") { setSubscriptionLoading(false); return; }
    let cancelled = false;
    setSubscriptionLoading(true);
    rssApi.get<{ feed: RssFeed; rule: RssRule | null }>(`/subscriptions/${encodeURIComponent(route.subscription)}`).then(record => { if (!cancelled) setSubscription(record); }).catch(e => { if (!cancelled) setSubscriptionError(rssError(e)); }).finally(() => { if (!cancelled) setSubscriptionLoading(false); });
    return () => { cancelled = true; };
  }, [route.subscription]);
  function openSubscription(id: string) { go({ tab: "feeds", subscription: id, edit: "", feed: null, job: null, q: "", status: "", site: "", source: "", target: "", page: 1 }); }
  ...
  function archive(kind: "feeds" | "rules", record: RssFeed | RssRule) {
    setConfirmation({ title: `删除${kind === "feeds" ? "订阅" : "下载规则"}`, description: `归档“${record.name}”将停止检查或匹配，并取消尚未添加到下载器的任务。已经发出的请求会继续确认结果。历史记录、下载器中的种子和文件会保留。`, label: "确认归档", dangerous: true, execute: () => mutation(`${kind}-${record.id}-archive-${record.version}`, async (id) => { await rssApi.archive(`/${kind}/${record.id}`, { expected_version: record.version, request_id: id }); if (kind === "feeds" && route.feed === record.id) { setSelectedFeed(null); go({ feed: null, page: 1 }); } }, "已归档，下载历史仍可查询。") });
  }
  ...
  if (route.subscription) return <div data-rss-page="true">{subscriptionLoading ? <ListSkeleton /> : subscriptionError ? <div className="space-y-4"><Button variant="outline" onClick={() => go({ subscription: "" })}><ArrowLeft />返回我的订阅</Button><ErrorBox action={<Button variant="outline" onClick={() => { go({ subscription: "", tab: "rules" }); }}>高级规则管理</Button>}>{subscriptionError}</ErrorBox></div> : (route.subscription === "new" || subscription) && <SubscriptionEditor key={route.subscription} initial={subscription} sites={sites} downloaders={downloaders} onBack={() => go({ subscription: "" })} onSaved={record => { setSelectedFeed(record.feed); setNotice(record.feed.enabled ? "订阅已开启。首次检查会收集已有资源，之后的新资源将按条件自动下载。" : "订阅已保存为暂停，准备好后可在列表中开启。"); go({ subscription: "", tab: "feeds", feed: record.feed.id }); void refreshOptions(); }} />}</div>;

  return <div className="space-y-5" data-rss-page="true">
    {!editing && <>
      {route.tab === "rules" && <h2 className="text-lg font-semibold">高级规则管理</h2>}
      <div className="flex flex-wrap items-center justify-between gap-3"><div role="tablist" aria-label="RSS 下载页面" className="flex min-w-0 gap-1 rounded-xl border border-border bg-card p-1">{([{ tab: "feeds", label: "我的订阅" }, { tab: "downloads", label: "下载记录" }] as const).map((tab) => <button type="button" role="tab" key={tab.tab} id={`rss-tab-${tab.tab}`} aria-selected={route.tab === tab.tab} aria-controls="rss-tab-panel" className={cn("min-h-11 whitespace-nowrap rounded-lg px-3 text-sm font-semibold transition-colors sm:px-5", route.tab === tab.tab ? "bg-accent text-primary" : "text-muted hover:bg-accent/60 hover:text-foreground")} onClick={() => selectTab(tab.tab)}>{tab.label}</button>)}</div><div className="flex flex-wrap gap-2">{route.tab === "feeds" && !route.feed && <Button className="h-11 px-4" onClick={() => openSubscription("new")}><Plus />添加订阅</Button>}{route.tab === "rules" && <Button className="h-11 px-4" onClick={() => openRule("new")}><Plus />新建规则</Button>}<Button className="h-11 px-3" variant="outline" onClick={() => { setRunsOpen(true); setRunPage(1); }}><Clock3 /><span className="hidden sm:inline">检查记录</span><span className="sm:hidden">记录</span></Button></div></div>
      <p className="text-sm leading-6 text-muted">{route.tab === "feeds" ? "管理每个订阅的来源与下载条件，符合的新资源会自动送到下载器。" : route.tab === "rules" ? "为订阅源设置筛选条件，符合的新资源会自动添加到指定下载器。" : "这里记录资源是否已添加到下载器，以及最新的下载进度。"}</p>
      {summary && <div className="flex flex-wrap items-center justify-between gap-2 text-xs"><div className="flex flex-wrap items-center gap-x-3"><button className="min-h-11 text-muted hover:text-foreground" onClick={() => { selectTab("feeds"); go({ tab: "feeds", feed: null, status: "running", source: "", target: "", q: "", page: 1 }); }}>运行中 <span className="ml-1 font-semibold tabular-nums text-foreground">{summary.running}</span></button><button className="min-h-11 text-muted hover:text-foreground" onClick={() => go({ tab: "feeds", feed: null, status: "paused", page: 1 })}>已暂停 <span className="ml-1 font-semibold tabular-nums text-foreground">{summary.paused}</span></button><span className="text-muted">需处理 <span className={cn("ml-1 font-semibold tabular-nums", summary.needs_attention ? "text-destructive" : "text-foreground")}>{summary.needs_attention}</span></span><button className="min-h-11 text-primary hover:underline" onClick={() => go({ status: "needs_attention", page: 1 })}>查看本页异常</button><span className="text-muted">等待添加 {summary.queued}</span></div><div className="flex items-center gap-2 text-muted"><span className="hidden sm:inline">{updatedAt ? `更新于 ${rssDate(updatedAt)}` : "等待数据"}</span><button className="flex size-11 items-center justify-center rounded-lg hover:bg-accent disabled:opacity-50" onClick={() => void loadRef.current()} disabled={refreshing} aria-label="刷新列表"><RefreshCw className={cn("size-4", refreshing && "motion-safe:animate-spin")} /></button></div></div>}
    </>}
    ...
    {editing ? editLoading ? <ListSkeleton /> : route.edit !== "new" && !editRule ? <Empty title="无法载入此规则" action={<Button className="h-11" variant="outline" onClick={() => selectTab("rules")}>返回下载规则</Button>}>规则可能已归档，或暂时无法连接服务。</Empty> : <RuleEditor key={route.edit} rule={editRule} feeds={feeds} downloaders={downloaders} feedId={route.feed ?? undefined} onBack={() => selectTab("rules")} onSaved={() => { setNotice("规则已保存。已有资源可在订阅源的资源列表中勾选补下。"); void refreshOptions(); selectTab("rules"); }} /> : <div role="tabpanel" id="rss-tab-panel" aria-labelledby={route.tab === "rules" ? undefined : `rss-tab-${route.tab}`} aria-label={route.tab === "rules" ? "高级规则管理" : undefined} className="space-y-4">
      {route.tab === "feeds" && route.feed && <div className="space-y-3"><Button className="-ml-3 h-11 border-transparent bg-transparent px-3" variant="outline" onClick={() => { setSelectedFeed(null); go({ feed: null, q: "", status: "", page: 1 }); }}><ArrowLeft />返回我的订阅</Button>{feed && <><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words text-xl font-semibold">{feed.name}</h2><p className="mt-2 text-xs leading-6 text-muted">{feed.site_name || "独立订阅源"} · {feed.enabled ? "已启用" : "已暂停"} · 每 {feed.interval_minutes} 分钟检查 · {feed.item_count} 条已收集</p></div><div className="flex flex-wrap gap-2"><Button className="h-11 px-3" variant="outline" disabled={!!busy || !feed.enabled} onClick={() => void check(feed)}><RefreshCw />立即检查</Button><Button className="h-11 px-3" onClick={() => openSubscription(String(feed.id))}><Pencil />订阅设置</Button></div></div><p className="break-all text-xs text-muted">{feed.url_display}</p>{!feed.initialized_at && <p className="rounded-lg bg-accent p-3 text-sm leading-6">等待第一次检查。已有资源会列在这里，供你按需勾选补下。</p>}{!feed.enabled && <p className="text-sm text-muted">订阅源和等待添加的任务已暂停。<button className="min-h-11 px-2 font-medium text-primary underline" onClick={() => toggleFeed(feed)}>恢复订阅源</button></p>}{feed.last_error && <ErrorBox action={<a className="inline-flex min-h-11 items-center text-primary underline" href="#/sites">查看站点配置<ArrowUpRight className="ml-1 size-4" /></a>}>{feed.last_error}</ErrorBox>}{!rules.some((rule) => rule.feed_ids.includes(feed.id) && rule.enabled) && <p className="text-xs leading-6 text-muted">还没有设置自动下载。打开「订阅设置」，选择下载条件和下载器。</p>}</>}</div>}
      <div className="grid grid-cols-2 gap-3 xl:flex xl:items-end"><div className="col-span-2 min-w-0 xl:flex-1"><label className="mb-2 block text-xs font-medium" htmlFor="rss-search">{route.tab === "feeds" && !route.feed ? "搜索订阅" : route.tab === "rules" ? "搜索规则" : "搜索资源"}</label><div className="relative"><Search className="pointer-events-none absolute left-3 top-3.5 size-4 text-muted" /><Input id="rss-search" className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={route.tab === "feeds" && !route.feed ? "按名称查找" : route.tab === "rules" ? "按规则名称查找" : "按资源标题查找"} /></div></div>{route.tab === "feeds" && !route.feed ? <div className="min-w-0 xl:w-44"><label className="mb-2 block text-xs font-medium" htmlFor="rss-site-filter">关联站点</label><Select id="rss-site-filter" value={route.site} onChange={(value) => go({ site: value, page: 1 })} options={[{ value: "", label: "全部站点" }, ...sites.map((site) => ({ value: String(site.id), label: site.name }))]} /></div> : route.tab !== "feeds" ? <div className="min-w-0 xl:w-44"><label className="mb-2 block text-xs font-medium" htmlFor="rss-source-filter">订阅源</label><Select id="rss-source-filter" value={route.source} onChange={(value) => go({ source: value, page: 1 })} options={[{ value: "", label: "全部来源" }, ...feeds.map((record) => ({ value: String(record.id), label: record.name }))]} /></div> : null}{route.tab === "downloads" && <div className="min-w-0 xl:w-44"><label className="mb-2 block text-xs font-medium" htmlFor="rss-target-filter">下载器</label><Select id="rss-target-filter" value={route.target} onChange={(value) => go({ target: value, page: 1 })} options={[{ value: "", label: "全部下载器" }, ...downloaders.map((record) => ({ value: String(record.id), label: record.name }))]} /></div>}<div className="min-w-0 xl:w-48"><label className="mb-2 block text-xs font-medium" htmlFor="rss-status-filter">{route.tab === "downloads" ? "添加状态" : "状态"}</label><Select id="rss-status-filter" value={route.status} onChange={(value) => go({ status: value, page: 1 })} options={route.tab === "downloads" ? jobStatuses : route.feed && route.tab === "feeds" ? itemStatuses : feedStatuses} /></div>{hasFilters && <Button className="h-11 px-3" variant="outline" onClick={clearFilters}><ListFilter />清除筛选</Button>}</div>
      ...
      {loading ? <ListSkeleton /> : !loaded && error ? null : currentList.items.length === 0 ? <div className="rounded-xl border border-border bg-card"><Empty title={hasFilters ? "没有符合筛选条件的记录" : route.tab === "feeds" ? route.feed ? "订阅源尚未收集到条目" : "把站点更新，变成自动下载" : route.tab === "rules" ? "尚未开启自动下载" : "还没有下载记录"} action={hasFilters ? <Button className="h-11" variant="outline" onClick={clearFilters}>清除筛选</Button> : route.tab === "feeds" && !route.feed ? <Button className="h-11" onClick={() => openSubscription("new")}><Plus />添加订阅</Button> : route.tab === "rules" ? <Button className="h-11" onClick={() => openRule("new")}><Plus />创建下载规则</Button> : undefined}>{hasFilters ? "调整关键词或状态，查看其他记录。" : route.tab === "feeds" ? route.feed ? "第一次检查后，资源会出现在这里。已有资源可勾选补下，新资源按启用的规则自动下载。" : "粘贴 RSS 链接，选好下载条件和下载器，一次完成订阅。首次检查不会自动下载已有资源。" : route.tab === "rules" ? "选择订阅源、填写关键词并指定下载器，之后符合条件的新资源就会自动下载。" : "自动下载和手动补下的任务都会显示在这里，可以查看添加结果和下载进度。"}</Empty></div> : route.tab === "feeds" && route.feed ? <div className="divide-y divide-border rounded-xl border border-border bg-card">{itemPage.items.map((item) => <article className="p-4 sm:p-5" key={item.id}><div className="flex items-start gap-3"><label className="flex min-h-11 w-5 shrink-0 items-start pt-1.5"><input className="size-4 accent-primary" type="checkbox" aria-label={`选择 ${item.title}`} checked={selectedItems.has(item.id)} disabled={!item.downloadable || (!selectedItems.has(item.id) && selectedItems.size >= 200)} onChange={() => toggleItem(item)} /></label><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><h3 className="min-w-0 flex-1 break-words text-sm font-semibold">{item.title}</h3><Status status={item.status} /></div><p className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted"><span>{rssSize(item.attributes.size_bytes)}</span><span>做种 {item.attributes.seeders ?? "未知"}</span><span>发现于 {rssDate(item.first_seen_at)}</span>{item.published_at && <span>发布于 {rssDate(item.published_at)}</span>}</p>{!item.downloadable && <p className="mt-2 text-xs text-muted">缺少可用种子定位，暂不能自动下载。</p>}<details className="group mt-2"><summary className="flex min-h-11 cursor-pointer list-none items-center gap-1 text-xs font-medium text-primary [&::-webkit-details-marker]:hidden">查看筛选结果与资源信息<ChevronDown className="size-4 transition-transform group-open:rotate-180" /></summary><div className="space-y-4 border-t border-border pt-3"><AttributeDetails attributes={item.attributes} />{item.decisions.length === 0 ? <p className="text-xs leading-6 text-muted">已有资源不会自动下载。想要这条资源，可以勾选后点击「补下所选条目」。</p> : item.decisions.map((decision) => <section key={decision.id} className="space-y-2 border-t border-border pt-3"><div className="flex flex-wrap items-center justify-between gap-2 text-xs"><button className="min-h-11 text-sm font-medium text-primary underline" onClick={() => openRule(String(decision.rule_id))}>{decision.rule_name}</button><Status status={decision.status} /></div><Evidence evaluation={decision.evaluation} /><p className="text-xs text-muted">判定：{rssDate(decision.checked_at)} · 使用当时的规则设置{decision.next_evaluate_at && ` · 下次判断 ${rssDate(decision.next_evaluate_at)}`}</p>{decision.job_id && <Button className="h-11 px-3" variant="outline" onClick={() => go({ tab: "downloads", feed: null, job: decision.job_id, status: "", q: "", page: 1 })}>查看下载任务</Button>}</section>)}</div></details></div></div></article>)}</div> : route.tab === "feeds" ? <div className="divide-y divide-border rounded-xl border border-border bg-card">{feedPage.items.map(record => {
        const attached = rules.filter(rule => rule.feed_ids.includes(record.id));
        const enabled = attached.filter(rule => rule.enabled);
        const configured = enabled.length > 0;
        return <article key={record.id} className="p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><button className="min-h-11 break-words text-left text-base font-semibold hover:text-primary" onClick={() => { setSelectedFeed(record); go({ feed: record.id, status: "", q: "", page: 1 }); }}>{record.name}</button><p className="text-xs leading-6 text-muted">{record.site_name || "独立 RSS"} · 每 {record.interval_minutes} 分钟检查 · {record.item_count} 条资源</p></div><span className={cn("pt-3 text-sm font-medium", record.last_error ? "text-destructive" : record.enabled && configured ? "text-jade" : "text-muted")}>{record.last_error ? "检查失败" : !record.enabled ? "已暂停" : configured ? "自动下载中" : "待完成设置"}</span></div>
          <div className="my-4 grid gap-4 border-y border-border py-4 sm:grid-cols-2"><div><p className="text-xs text-muted">下载条件</p><p className="mt-1 break-words text-sm leading-6">{attached.length ? attached.map(rule => `${rule.filters.match_all ? "全部标题" : rule.filters.include.join(rule.filters.include_mode === "all" ? " + " : " / ") || rule.filters.include_regex || "高级条件"}${rule.filters.free_only ? " · 仅免费" : ""}${rule.filters.hr_policy === "require_clear" ? " · 无 H&R" : ""}`).join("；") : "尚未设置，暂不会自动下载"}</p></div><div><p className="text-xs text-muted">下载到</p><p className="mt-1 break-words text-sm leading-6">{attached.map(rule => rule.downloader_name || "未选择下载器").join("、") || "待选择下载器"}</p>{attached[0]?.options.save_path && <p className="mt-1 break-all text-xs text-muted">{attached[0].options.save_path}</p>}</div></div>
          {record.last_error && <p role="status" className="mb-3 break-words text-sm leading-6 text-destructive">{record.last_error} · <a className="underline" href="#/sites">检查站点配置</a></p>}
          {attached.some(rule => rule.last_error) && <p className="mb-3 text-sm text-destructive">{attached.filter(rule => rule.last_error).map(rule => rule.last_error).join("；")}</p>}
          <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-xs leading-6 text-muted">{record.initialized_at ? `最近检查 ${rssDate(record.last_checked_at)}` : "首次检查只收集资源，不自动下载"}{record.pending_count > 0 && ` · ${record.pending_count} 条待处理`}</p><div className="flex flex-wrap gap-2"><Button className="h-11 px-3" variant="outline" onClick={() => { setSelectedFeed(record); go({ feed: record.id, status: "", q: "", page: 1 }); }}>查看资源</Button><Button className="h-11 px-3" variant={configured ? "outline" : "default"} onClick={() => openSubscription(String(record.id))}><Pencil />{configured ? "设置" : "完成设置"}</Button><button disabled={!!busy} className="flex size-11 items-center justify-center rounded-lg text-muted hover:bg-accent disabled:opacity-50" aria-label={`${record.enabled ? "暂停" : "恢复"}${record.name}`} onClick={() => toggleFeed(record)}>{record.enabled ? <Pause className="size-4" /> : <Play className="size-4" />}</button><button className="flex size-11 items-center justify-center rounded-lg text-muted hover:bg-accent hover:text-destructive disabled:opacity-50" disabled={!!busy} aria-label={`删除订阅${record.name}`} onClick={() => archive("feeds", record)}><Archive className="size-4" /></button></div></div>
        </article>;
      ...
      {route.tab === "feeds" && !route.feed && <div className="flex justify-end"><button className="min-h-11 text-xs text-muted underline hover:text-primary" onClick={() => selectTab("rules")}>高级规则管理</button></div>}
