...
接收服务与现有 Web 服务共用监听地址和端口，路径为 `/dav/ptd/`。Linux 和 Docker 的 Web 服务均适用；桌面版没有对外 Web 端口，如需使用本功能，请使用 Linux 版本。启用、关闭和密码变更在配置保存后生效，后台每秒检查同步队列。旧配置中的独立监听地址和端口不再使用，原有账号、密码及同步记录保留。
