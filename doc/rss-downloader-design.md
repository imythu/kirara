...
| 标题筛选 | 包含词、排除词、可选正则；完全无筛选须显式选择「接收所有标题」 |
...
复用共享 Dialog：名称、RSS 地址、关联站点、检查间隔；连接设置收起代理选项；常用设置旁提供用途说明，较长的解释按需展开。站点 Cookie/API Key 从站点配置引用，不在此复制一套账户表单。
...
预览区固定展示：样例总数、符合数、被排除数、信息不足数、每条的原因。匹配字符可以高亮，但不能只用颜色表达通过与拒绝。默认预览使用本地已抓取数据并标注时间；「读取最新资源」才访问远端。
...
| 修改规则名称 | 不改变匹配生效范围 |
| 使用其他 RSS 地址 | 新增独立订阅源；现有地址创建后不可修改，接口拒绝替换且不改变历史和任务 |
...
恢复规则时，活动源以当前发现序号建立新边界；恢复一个未在暂停期间抓取的源时，首个成功响应中的新增条目被视为暂停期间的基线。界面用「暂停期间出现的资源不会自动下载，可在资源列表中勾选补下」说明恢复行为。

筛选变更前已入队的任务使用入队时的规则和下载选项快照。编辑页提示已有待投递任务数量，并提供先暂停查看的入口。筛选变更不会悄悄取消或改写既有任务；最新凭据、启停状态、容量检查仍在提交前读取。

RSS 地址创建后固定，编辑表单仅展示脱敏地址。保存接口拒绝替换地址；更换链接需要新增订阅源。保留已有代次字段与旧数据处理逻辑，以兼容历史记录，不提供变更代次的用户操作。

历史补下限定为用户在已保存条目中选中的资源，选择规则后重新预览，并使用相同条件与去重流程。它只覆盖“早于生效边界”，不绕过种子校验、属性限制和下载器检查。自动任务不会因为清理日志、恢复进程或修改发布时间而重放历史。

## 7. 抓取与解析

请求由 Rust 服务发起。关联站点时复用该站点的代理选择，并使用其接口查询属性和获取种子；RSS 地址允许与该接口不同源。认证和私密请求头只用于与站点配置同源的请求；未关联时仅使用源 URL 本身及明确的代理设置。不猜测站点归属，不把站点凭据发送到不同 origin。
...
1. 优先使用 RSS enclosure 或有明确种子类型证据的 link，由服务端取回并校验 bencode；普通详情页不直接当作种子文件。
2. 链接失效或未提供链接时，使用稳定种子 ID 和关联站点已保存的认证信息，获取同一个种子。
3. 站点无法提供取种请求时，重新抓取 RSS，按原条目身份查找更新后的下载链接。

缺失 GUID 时依次采用稳定站点种子 ID、可识别的详情定位、下载定位的指纹。只对有明确站点规则的 URL 做参数规范化；通用地址保留参数语义，不通过随意删除 query 制造错误去重。没有可靠身份的条目展示为不可自动处理。

短期签名地址失效且无法通过关联站点取种时，重新抓取源一次，根据稳定条目身份更新定位后重试；找不到原条目则明确失败，不使用相似标题替换。无法识别站点 API 时，也不将 M-Team 等站点的详情链接猜成可下载地址。

网络或服务临时故障沿用正常重试，429 遵守冷却时间，不触发站点取种回退。
...
创建时必须填写地址；更新时省略、空值或相同地址表示保留，不同地址返回 422 且不修改任何记录；RSS 与关联站点 API 可以不同源，关联时检查站点存在，请求时按目标地址决定使用哪些凭据。HTTP 请求仅允许 HTTP/HTTPS，禁止 URL userinfo；同源重定向才继承凭据，跨源默认拒绝并说明原因。用户配置的内网源可以使用，不因部署在内网就一律拒绝。
