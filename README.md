...
- [RSS 下载](doc/rss-downloader.md)：订阅种子源，按标题与属性规则自动收取新增资源，预览判定并按需补下历史条目
- 管理 PT 刷流任务
...
| 系统 | 安装包 | 使用方式 |
| --- | --- | --- |
| Windows x64 | `kirara-2.4.4-x86_64-pc-windows-msvc-setup.exe` | 运行安装程序，完成后从开始菜单打开 Kirara |
| macOS Apple Silicon | `kirara-2.4.4-aarch64-apple-darwin.dmg` | 打开映像，将 Kirara 拖入“应用程序”，然后打开 |
...
在「自动任务 → 定时任务」创建 HTTP 请求任务。默认使用人类可读的下拉间隔（分钟、小时、天、周），也可自定义间隔；Cron 模式支持五字段表达式、固定 UTC 时区偏移及接下来三次执行时间预览。例如 `0 9 * * MON-FRI` 表示工作日 09:00，数字星期 `0` / `7` 均为周日；日和星期同时限制时，任一匹配即执行。

HTTP 面板支持方法、URL、查询参数、请求头，认证支持无认证、Bearer Token、Basic Auth、API Key（请求头或查询参数）与 Cookie。请求体支持 JSON、URL 编码表单、multipart 文本/文件表单、纯文本、XML、HTML、二进制文件及自定义 Content-Type；正文或表单内容与文件合计最大 256 KiB。还支持 1–300 秒超时、重定向和预期状态码。发送方式支持 HTTP 客户端和 Browserless 浏览器。HTTP 模式可选择使用全局代理；浏览器模式复用自动签到工具的 Browserless 地址和 Token，可单独选择连接浏览器时是否使用全局代理。该开关仅控制云母到浏览器服务的连接，浏览器到目标站点的出口由服务配置决定。浏览器服务需支持 /function API，请求在独立会话中发送，不执行目标页面脚本；支持原有认证、表单和文件内容。默认任意 2xx 为成功，执行记录保留状态码和耗时，不保存响应正文。编辑名称或计划默认保留原请求；点击「载入已保存配置」可查看和修改请求与认证，认证值默认隐藏，可主动显示生成的最终认证信息。列表和执行记录不回传凭据，载入与预览接口禁止缓存。请求预览复用实际执行的构建逻辑，不发送网络请求；multipart 展示字段/文件摘要，执行时重新生成 boundary。上传文件随任务保存，每次执行发送相同内容。Cookie 认证可通过输入框内的悬浮下拉搜索已有站点，选择后复制当前 Cookie；不会自动同步站点后续更新。

HTTP 任务可通过「导入 cURL」粘贴浏览器 Copy as cURL（Bash）命令，先解析并查看完整值，再应用到请求配置、保存任务。支持多行与 Bash 引号、方法、请求头、Cookie、Basic/Bearer、JSON/原文/URL 编码正文和 multipart 文本字段，以及超时、重定向；保留当前计划、发送方式、代理和状态码设置。编辑已有任务时先载入已保存配置。解析不执行命令、不发送请求、不读取本地文件；变量、管道、文件引用及不支持的选项会明确报错，文件可在导入后通过面板添加。

任务支持暂停/启用、手动执行和最近 100 条记录。首次执行等待一个完整间隔，保存或启用时重新计算计划；手动执行不改变计划，暂停不会取消正在发送的请求。同一任务不重叠执行，最多同时运行 8 个任务。服务需保持运行；停机错过的计划恢复后最多补执行一次，未完成记录标记为中断、结果未知。失败请求不自动重试。
