...
RSS configuration presents source, download conditions and destination as one subscription. Its inline editor uses three sequential steps, shared bordered fields and violet active-step text; the primary navigation pairs subscriptions with download records, with advanced rule management as a secondary link. One-sentence explanations accompany fields; optional filters and execution settings use inline disclosures, opening when already configured. Saved RSS addresses appear as masked text. At 1280px and above, a 280px summary sits beside the form; narrower layouts show that summary only after the preview and confirmation step. List filters use two columns on narrow screens, with search spanning both; row actions wrap and conditions and destinations remain readable. These are RSS-specific arrangements, not a new global layout rule.
